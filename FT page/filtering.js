window.data = {
  ...
  Resources: {
    ...
    Options: [
      ...
      },
      {
        ...
        Content: 'Learn how to configure built-in search behavior for quick record lookup.',
        ReadGuideORViewDemoTitle: 'Read Guide',
        ReadGuideORViewDemoLink: 'https://ej2.syncfusion.com/react/documentation/grid/searching'
      },
      {
  ...
  FAQ: {
    Eyebrow: 'FAQ',
    MainTitle: 'Common Questions About Grid Search, Filter, and Sort',
    Subtitle: '',
    Options: [
      {
      ...
      },
      {
        ...
        Answer: 'Yes. Users can apply filters and sort columns at the same time for a highly focused data view.'
      },
      {
      ...
      },
      {
        ...
        Answer: 'Yes. The grid supports rich filtering experiences across different column types.'
      },
      {
  ...
  RelatedArticles: {
    ...
    Subtitle: 'Deepen your understanding of grid search, filtering, and sorting capabilities.',
    Options: [
      {
      ...
      {
        Title: 'Search Records on Each Keystroke',
        Content: 'Enable instant Grid searching without pressing the Enter key to deliver a faster and more responsive search experience.',
        ReadArticleLink: 'https://support.syncfusion.com/kb/article/9696/how-to-search-the-records-in-grid-on-each-keystroke'
      },
      {
        Title: 'Filter Multiple Records Using Filter Bar Template',
        Content: 'Learn how to filter multiple values simultaneously using a customized filter bar template in the JavaScript Grid.',
        ReadArticleLink: 'https://support.syncfusion.com/kb/article/10143/how-to-filter-multiple-records-using-the-filter-bar-template-in-javascript-grid'
      },
      {
        Title: 'Perform Advanced Filtering Using Custom Queries',
        Content: 'Implement advanced filtering requirements by applying custom queries and filtering logic within the JavaScript Grid.',
        ReadArticleLink: 'https://support.syncfusion.com/kb/article/9690/how-to-perform-advanced-filtering-in-grid-using-custom-queries'
      },
      {
        Title: 'Search Date Type Columns in Grid',
        Content: 'Learn techniques for searching records within date-type columns and improving date-based record discovery.',
        ReadArticleLink: 'https://support.syncfusion.com/kb/article/9695/how-to-perform-searching-in-date-type-column'
      }
    ]
  }
};
...
document.getElementsByClassName('hero')[0].innerHTML = `
  <div class="container">
    ...
    <div class="hero-ctas">
      <a href="${data.HeroWithCTA.ViewDemoLink}" target="_blank" rel="noopener noreferrer" class="btn btn-primary">${data.HeroWithCTA.ViewDemo}</a>
      <a href="${data.HeroWithCTA.ExploreDocLink}" target="_blank" rel="noopener noreferrer" class="btn btn-secondary">${data.HeroWithCTA.ExploreDoc}</a>
    </div>
  </div>
`;
...
document.getElementsByClassName('capabilities-section')[0].innerHTML = `
  <div class="container">
...
document.getElementsByClassName('resources')[0].innerHTML = `
  <div class="container">
    ...
    <div class="resources-grid">
      ${data.Resources.Options.map(item => `
        <div class="resource-card">
          <h4>${item.Title}</h4>
          <p>${item.Content}</p>
          <a href="${item.ReadGuideORViewDemoLink}" target="_blank" rel="noopener noreferrer">${item.ReadGuideORViewDemoTitle} →</a>
        </div>
      `).join('')}
    </div>
  </div>
...
document.getElementsByClassName('faq')[0].innerHTML = `
  <div class="container">
...
document.getElementsByClassName('related-articles')[0].innerHTML = `
  <div class="container">
    ...
    <div class="articles-grid">
      ${data.RelatedArticles.Options.map(item => `
        <div class="article-card">
          <h4>${item.Title}</h4>
          <p>${item.Content}</p>
          <a href="${item.ReadArticleLink}" target="_blank" rel="noopener noreferrer" class="article-link">Read article →</a>
        </div>
      `).join('')}
    </div>
  </div>
